...
In the author's last paper we introduced \emph{Mirror-Match} and \emph{Mirror-Match with Clustering}, two algorithms that we showed to perform better than the state of the art. The novel contributions of this paper consists of two new algorithms completing a framework of four related matching algorithms as well as a theoretical foundation for comparing the algorithms. We use this foundation to formally prove a ranking in terms of performance for the different algorithm which enables us to understand why \emph{Mirror-Match} performs better than \emph{Ratio Match} in the first place. In addition we test these results on a much larger dataset consisting of over 3000 image pairs.
...
as assumed by \emph{Ratio-Match}.}
    \item{We assume that distances of feature descriptors are well 
            ...
            $F_{t}$, we have $\forall f_i \in F_{proposal}, 
