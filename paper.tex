...
%The geometric approach tries to find unique keypoints in an image pair and matches them based on their location.  The descriptor centric approach on the other hand uses the local image information around the keypoint to create a descriptor and matches descriptors based on their similarity.
...
\item{Let $\tau \in [0 \ldots 1]$ be a threshold used to decide whether to keep a match.} %We keep a match only if it has a \emph{uniqueness ratio} lower than $\tau$}
...
The algorithms \emph{Ratio-Match}, \emph{Self-Match} and \emph{Both-Match} all find the best match to a given \emph{query feature} only in the \emph{target image}. They differ by the feature set used as the \emph{baseline set}. While \emph{Ratio-Match} uses features from the \emph{target image}, \emph{Self-Match} draws the \emph{baseline set} from the \emph{query image}. Finally \emph{Both-Match} uses the conjunction of features from both images. 

Each of these three algorithms has an `extended' case where the proposal set is replaced with the conjunction of features from both the \emph{target image} and the \emph{query image}. The extended version of \emph{Self-Match} is \emph{Self-Match-Ext} and the extended version of \emph{Ratio-Match} is \emph{Ratio-Match-Ext}. Finally we call the extended version of \emph{Both-Match} for \emph{Mirror-Match} because it was under this name that the algorithm was originally introduced by the authors in~\cite{arnfred2013mirror}.
